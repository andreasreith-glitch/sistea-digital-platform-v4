...
export default function EnergyCostCalculator({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl">
      <div className="grid gap-8 bg-slate-50 p-6 sm:p-8 lg:grid-cols-[.9fr_1.1fr] lg:p-10">
        <div>
          <div className="text-xs font-black uppercase tracking-[.18em] text-cyan-700">{t.eyebrow}</div>
          <h3 className="mt-3 text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">{t.title}</h3>
          <p className="mt-4 leading-7 text-slate-600">{t.text}</p>

          <div className="mt-7 grid gap-4 sm:grid-cols-2">
            <label className="grid gap-2 text-sm font-bold text-slate-700">
              {t.bill}
              <input type="number" min="0" step="0.01" value={bill} onChange={(e) => setBill(Number(e.target.value))} className="rounded-xl border border-slate-300 bg-white px-4 py-3 text-base font-semibold outline-none focus:border-cyan-500" />
            </label>
            <label className="grid gap-2 text-sm font-bold text-slate-700">
              {t.currency}
              <select value={currency} onChange={(e) => setCurrency(e.target.value as Currency)} className="rounded-xl border border-slate-300 bg-white px-4 py-3 text-base font-semibold outline-none focus:border-cyan-500">
            ...
            <label className="grid gap-2 text-sm font-bold text-slate-700">
              {t.kwh}
              <input type="number" min="0" step="1" value={kwh} onChange={(e) => setKwh(Number(e.target.value))} className="rounded-xl border border-slate-300 bg-white px-4 py-3 text-base font-semibold outline-none focus:border-cyan-500" />
            </label>
            <label className="grid gap-2 text-sm font-bold text-slate-700">
              {t.months}
              <select value={months} onChange={(e) => setMonths(Number(e.target.value))} className="rounded-xl border border-slate-300 bg-white px-4 py-3 text-base font-semibold outline-none focus:border-cyan-500">
            ...
            <label className="grid gap-2 text-sm font-bold text-slate-700 sm:col-span-2">
              {t.demand}
              <input type="number" min="0" step="0.1" value={demand || ""} onChange={(e) => setDemand(Number(e.target.value))} placeholder="0" className="rounded-xl border border-slate-300 bg-white px-4 py-3 text-base font-semibold outline-none focus:border-cyan-500" />
            </label>
          </div>
        </div>

        <div className="rounded-2xl bg-slate-950 p-6 text-white sm:p-7">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-xl border border-white/10 bg-white/5 p-5 sm:col-span-2">
              <div className="text-xs font-black uppercase tracking-[.16em] text-slate-400">{t.current}</div>
              <div className="mt-2 text-4xl font-black text-cyan-300">{money(values.annual)}</div>
            </div>
            <div className="rounded-xl border border-white/10 bg-white/5 p-5">
              <div className="text-xs font-black uppercase tracking-[.14em] text-slate-400">{t.unit}</div>
              <div className="mt-2 text-xl font-black">{unitMoney(values.unitCost)} / kWh</div>
            </div>
            <div className="rounded-xl border border-white/10 bg-white/5 p-5">
              <div className="text-xs font-black uppercase tracking-[.14em] text-slate-400">{t.demandLabel}</div>
          ...
          <div className="mt-5 grid gap-3">
            {scenarios.map((rate) => {
              ...
              return (
                <div key={rate} className={`rounded-xl border p-5 ${rate === 0.4 ? "border-cyan-400/60 bg-cyan-400/10" : "border-white/10 bg-white/5"}`}>
                  <div className="flex items-center justify-between gap-4">
                    <div className="text-sm font-black uppercase tracking-[.12em] text-slate-300">{t.scenario}</div>
                    <div className={`text-2xl font-black ${rate === 0.4 ? "text-cyan-300" : "text-white"}`}>{rate === 0.4 ? `~${pct}%` : `${pct}%`}</div>
                  </div>
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <div><div className="text-xs text-slate-400">{t.saving}</div><div className="mt-1 text-lg font-black text-emerald-300">{money(saving)}</div></div>
                    <div><div className="text-xs text-slate-400">{t.after}</div><div className="mt-1 text-lg font-black">{money(after)}</div></div>
                  </div>
                </div>
              );
            })}
      ...
      <div className="border-t border-slate-200 p-6 sm:p-8 lg:flex lg:items-center lg:justify-between lg:gap-8">
        <p className="max-w-3xl text-sm leading-6 text-slate-500">{t.disclaimer}</p>
        <div className="mt-5 flex flex-col gap-3 sm:flex-row lg:mt-0 lg:flex-none">
          ...
          <a href={mailto} className="inline-flex items-center justify-center rounded-xl border border-slate-300 bg-white px-5 py-3 font-black text-slate-900 transition hover:bg-slate-50">{t.email}</a>
        </div>
      </div>
    </div>
  );
